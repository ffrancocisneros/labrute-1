...
const Server = {
  ...
  Tournament: {
    ...
    getCopaDelRey: (date: string) => Fetch<TournamentsGetCopaDelReyResponse>(`/api/tournament/copa-del-rey/${date}`),
    getSpecial: ({
      name,
      date,
    }: { name: string, date: string }) => Fetch<TournamentsGetSpecialResponse>(`/api/tournament/special/${name}/${date}`),
